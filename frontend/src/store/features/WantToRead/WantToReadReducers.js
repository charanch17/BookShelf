...
export const addWantToReadBook = createAsyncThunk(
    "WantToRead/post",async(payload)=>{
        ...
        if(!alldocs.includes(payload.bookData.id)){
            resp =await addDoc(WantToReadCollectionRef,{uid:payload.uid,...payload.bookData})
            console.log(resp.id,payload.bookData)
