...
export const fetchReadBooks= createAsyncThunk(
    "Read/get" , async(payload)=>{
...
export const addReadBook = createAsyncThunk(
    "Read/post",async(payload)=>{
        ...
        if (payload.actiontype !== "move") {
            ...
            var q3 = query(currentReadingCollectionRef, where("uid", "==", payload.uid));
        ...
        if(!alldocs.includes(payload.bookData.id)){
            resp = await addDoc(ReadCollectionRef,{uid:payload.uid,...payload.bookData})
            console.log(resp,"as")
...
export const removeReadBook = createAsyncThunk(
    ...
    async(payload)=>{
        ...
        console.log(resp,payload.docId,docRef)
